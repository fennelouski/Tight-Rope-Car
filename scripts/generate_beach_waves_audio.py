#!/usr/bin/env python3
"""Synthesize a short seamless beach waves ambience loop (distinct from ocean_waves)."""
...
WAV_PATH = OUT_DIR / "beach_waves.wav"
CAF_PATH = OUT_DIR / "beach_waves.caf"
...
def generate_samples() -> list[float]:
    ...
    for i in range(count):
        ...
        brown = brown * 0.982 + white * 0.1

        wave_slow = math.sin(t * 0.42 * 2 * math.pi) * 0.32
        wave_fast = math.sin(t * 1.1 * 2 * math.pi + 0.5) * 0.06
        surf = brown * 0.45 + wave_slow + wave_fast

        sample = surf
        fade = min(1.0, t / 0.35, (DURATION_SEC - t) / 0.35)
        samples.append(sample * fade * 0.44)
